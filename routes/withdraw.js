...
router.get('/', function(req, res, next) {
  ...
  if (req.query.tokenNumber) {
    tokenNumber = parseInt(req.query.tokenNumber);
  }

  console.info(tokenNumber/100*1.2);
  console.info("value: " +tokenNumber);
  Registery.deployed().then( async (instance) => {
    ...
    var privateKey = new Buffer(priateKeys[0].toString(), 'hex');
    ...
    var rawTx = {
      ...
      gasLimit: '0x3d0900',
      to: accounts[1],//转账到哪个账户
      value: web3.toHex(web3.toWei(tokenNumber/100*1.2,'ether')),//以太币数量
      data: ''
    };
    ...
    return instance.transfer(accounts[0], web3.toWei(tokenNumber,'ether'),{from:accounts[1]});
